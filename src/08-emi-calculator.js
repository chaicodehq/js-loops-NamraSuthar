...
export function calculateEMI(principal, monthlyRate, emi) {
  ...
  if (typeof principal !== 'number' || principal <= 0) return error;
  if (typeof monthlyRate !== 'number' || monthlyRate <= 0) return error;
  if (typeof emi !== 'number' || emi <= 0) return error;

  if (emi <= principal * monthlyRate) return error;
